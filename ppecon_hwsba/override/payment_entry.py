...
class CustomPaymentEntry(AccountsController):
	...
	def build_gl_map(self):
		...
		if self.custom_consolidate_vat_advance:# MiM
			if  self.project:
				self.add_receive_gl_entries(gl_entries)
			else:
				frappe.throw(_("Project is mandatory for Payment Entry To Complete Consolidation...Please Set Project"))

		return gl_entries

	def add_receive_gl_entries(self, gl_entries):
		if self.payment_type == "Receive":
			project_doc = frappe.get_doc("Project",self.project)
			debit_account = project_doc.custom_vat_advance_account
			if not debit_account:
				frappe.throw(_("VAT Advance Account is mandatory for Payment Entry...Please Set it In Project {0}".format(self.project)))
			credit_account = project_doc.custom_vat_account
			if not credit_account:
				frappe.throw(_("VAT Account is mandatory for Payment Entry...Please Set it In Project {0}".format(self.project)))

			vat_percent = frappe.db.get_value("Account",project_doc.custom_vat_account, "tax_rate") or 0
			if not vat_percent or vat_percent == 0:
				frappe.throw(_("Tax Rate is mandatory for Payment Entry...Please Set it In Account {0}".format(credit_account)))

			amount = (flt(self.paid_amount)/((flt(vat_percent)/100) + 1)) * (flt(vat_percent)/100)
