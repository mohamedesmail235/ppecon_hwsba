...
class CustomSalesInvoice(SellingController):
    ...
    def get_gl_entries(self, warehouse_account=None):
        ...
        if self.custom_abstract_consolidation:
            self.create_vat_advance_entry(gl_entries)
    ...
    def create_vat_advance_entry(self,gl_entries):
        # =================================Project Validation======================================
        project_doc = frappe.get_doc("Project", self.project)
        advance_account = project_doc.custom_advance_account
        vat_account = project_doc.custom_vat_account
        vat_advance_account = project_doc.custom_vat_advance_account

        if not advance_account:
            frappe.throw(_("Advance Account is mandatory for ...Please Set it In Project {0}".format(self.project)))
        if not vat_account:
            frappe.throw(_("VAT Account is mandatory...Please Set it In Project {0}".format(self.project)))
        if not vat_advance_account:
            frappe.throw(_("VAT Advance Account is mandatory...Please Set it In Project {0}".format(self.project)))

        vat_percent = frappe.db.get_value("Account", vat_account, "tax_rate") or 0
        if vat_percent == 0:
            frappe.throw(_("Tax Rate is mandatory...Please Set it In Account {0}".format(vat_account)))


        vat_amount = (flt(self.total_advance) / ((flt(vat_percent) / 100) + 1)) * (flt(vat_percent) / 100)

        # =================================Credit======================================
        gl_entries.append(
            self.get_gl_dict(
                {
                    "account": vat_advance_account,
        ...
        base_grand_total -= (flt(self.total_advance) + flt(self.custom_grantee_value))
        ...
        gl_entries.append(
            self.get_gl_dict(
                {
                    "account": self.custom_grantee_account,
                    "party_type": "Customer",
                    "party": self.customer,
                    "due_date": self.due_date,
                    "against": self.against_income_account,
                    "debit": self.custom_grantee_value,
                    "debit_in_account_currency": self.custom_grantee_value,
            ...
            self.get_gl_dict(
                {
                    ...
                    "due_date": self.due_date,
                    "against": self.against_income_account,
                    "debit": self.total_advance,
                    "debit_in_account_currency": self.total_advance,
                    "against_voucher": against_voucher,
                    "against_voucher_type": self.doctype,
                    "cost_center": self.cost_center,
                    "project": self.project,
    ...
    def validate_outstanding_amount(self):
        if self.custom_grantee_value and self.docstatus == 0:
            self.outstanding_amount = flt(self.grand_total) - (flt(self.custom_grantee_value) + flt(self.total_advance))
